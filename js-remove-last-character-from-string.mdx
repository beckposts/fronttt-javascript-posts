...
const yourString = "Remove";
const newString = yourString.substring(0, str.length - 1);
console.log(newString); // Prints Remov
...
`substr(0, str.length - 1)` will remove last character, starts at 0 and uses the length of 
string length -1 as the lenth.

<CodeEditor id={3} markdownCode={`

const yourString = "Remove";
const newStr = yourString.substr(0, str.length - 1);
...
const newString = yourString.slice(0, str.length - 1);
